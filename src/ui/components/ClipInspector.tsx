/**
 * Inspector for the selected clip: per-clip audio (volume + mute) and, for clips
  ...
  return (
    <aside className="inspector">
      ...
      <label className="inspector__field">
        ...
        <input
          type="range" min={0.3} max={1.7} step={0.01}
        ...
        <>
          ...
          <div className="inspector__row">
            <label className="inspector__field">
              <span>Entrada</span>
              <select
                value={clip.enter?.type ?? 'none'}
                onChange={(e) =>
                  setClipAnim(clip.id, { enter: { type: e.target.value as AnimType, durationSec: animDur } })
                }
              >
            ...
            <label className="inspector__field">
              ...
              <select
                ...
                onChange={(e) =>
                  setClipAnim(clip.id, { exit: { type: e.target.value as AnimType, durationSec: animDur } })
                }
              >
                {ANIM_OPTIONS.map(([v, l]) => (
                  <option key={v} value={v}>{l}</option>
